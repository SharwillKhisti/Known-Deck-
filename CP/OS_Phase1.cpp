    ...
    while (true) {
        ...
        if (op1 == 'G' && op2 == 'D') {
            ...
            if (op1 == 'L' && op2 == 'R') {
                ...
                cout << "  -> [EXEC] NR" << operand << " : R AND M[" << operand << "] -> R\n";
                for (int i = 0; i < 4; i++)
                    R[i] = (R[i] == '1' && M[operand][i] == '1') ? '1' : '0';
            }
            else if (op1 == 'O' && op2 == 'R') {
                cout << "  -> [EXEC] OR" << operand << " : R OR M[" << operand << "] -> R\n";
                for (int i = 0; i < 4; i++)
                    R[i] = (R[i] == '1' || M[operand][i] == '1') ? '1' : '0';
            }
            else if (op1 == 'X' && op2 == 'R') {
                cout << "  -> [EXEC] XR" << operand << " : R XOR M[" << operand << "] -> R\n";
                for (int i = 0; i < 4; i++)
                    R[i] = (R[i] != M[operand][i]) ? '1' : '0';
            }
